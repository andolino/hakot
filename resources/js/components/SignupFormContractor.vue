<template>
	<div >
		<div v-if="showFormSignUp">
			...
			<form @submit.prevent="signupContractor">
			...
			<p class="text-center mt-3">Already have an account? <a href="javascript:void(0);" @click="showContractorLogin" class="text-warning">Login</a></p>
		</div>
		<div v-if="showFormLogin">
			<ContractorLogin :base_url="base_url"/>
			<p class="text-center mt-3">Create an account? <a href="javascript:void(0);" @click="showContractorSignUp" class="text-warning">Signup</a></p>
		</div>
	</div>
</template>
    ...
    export default {
			...
			methods: {
				signupContractor(){
					...
					axios.post('/hakot/api/register/contractor', data).then(() => {
						this.form.reset();
					}).catch((error) => {
