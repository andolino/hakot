<template>
  <div>
		<div v-if="showFormSignUp">
			...
			<form @submit.prevent="signupTruckers">
			...
			<p class="text-center mt-3">Already have an account? <a href="javascript:void(0);" @click="showTruckersLogin" class="text-warning">Login</a></p>
		</div>
		<div v-if="showFormLogin">
			<TruckersLogin :base_url="base_url"/>
			<p class="text-center mt-3">Create an account? <a href="javascript:void(0);" @click="showTruckersSignUp" class="text-warning">Signup</a></p>
		</div>
	</div>
...
<script>
		import TruckersLogin from './TruckersLogin';
    export default {		
			name: "SignupFormTruckers",
			props: [ 'base_url' ],
			components: {
				TruckersLogin
			},
			...
			methods: {
				signupTruckers(){
					...
					axios.post('/hakot/api/register/truckers', data).then(() => {
						this.form.reset();
					}).catch((error) => {
